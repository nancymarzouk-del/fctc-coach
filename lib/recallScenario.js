...
// Study time SHORTENS as the level rises: Level 1 is generous, Level 5 is a
// brief-but-readable exposure. Difficulty comes from information density and
// recall demands, not from a punishingly short clock.
const STUDY_MS = { 1: 22000, 2: 17000, 3: 13000, 4: 10000, 5: 8000 };

export function generateBoard(seed = Date.now(), difficulty = 2, preferKind = null) {
  const rng = makeRng(seed);
  const n = detailBudget(difficulty);
  // Steer toward a board kind that exercises a recently-missed detail type
  // (the CONTENT still varies with the seed, so scenes never repeat), else random.
  const kind = (preferKind && BOARD_TYPES.includes(preferKind)) ? preferKind : pick(rng, BOARD_TYPES);
  const board = BOARD_BUILDERS[kind](rng, n);
  return {
    id: uid(),
    seed,
    difficulty,
    durationMs: STUDY_MS[difficulty] || STUDY_MS[2],
    detailCount: n,
    ...board,
  };
}
...
export function generateRecallQuestions(board, seed = Date.now(), count = 6) {
  ...
  // Level-aware mix: at low levels favour basic recall (identity/count); at high
  // levels favour the harder dimensions (sequence, direction, personnel position).
  const HARD = new Set(['sequence', 'directional', 'personnel']);
  const lvl = board.difficulty || 2;
  const weighted = shuffle(rng, pool).sort((a, b) => {
    const score = (q) => (lvl >= 3 ? (HARD.has(q.subskill) ? 0 : 1) : (HARD.has(q.subskill) ? 1 : 0));
    return score(a) - score(b);
  });
  // finalize: slice to count, attach ids + domain
  return weighted.slice(0, count).map(q => ({
    id: uid(),
    domain: 'recall',
    subskill: q.subskill,
