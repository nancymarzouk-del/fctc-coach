...
export function makeRng(seed = Date.now()) {
  // Avalanche the seed first (SplitMix32-style) so small/sequential seeds
  // don't produce clustered first outputs.
  let s = (seed >>> 0) || 1;
  s = (s ^ 0x9e3779b9) >>> 0;
...
import { VISUAL_BUILDERS } from './mechanicalVisuals.mjs';
import { MATH_DIAGNOSIS_BUILDERS } from './mathDiagnosis.mjs';
import { withReadingCoaching, readingSkillFor } from './readingCoach.mjs';
import { provenanceFor } from './fctcBlueprint.mjs';
...
// ============================================================================
const mechanical = {
  // Levers, pulleys, gears, belts, hydraulics, and load-balance delegate to the
  // tested, mechanically-correct visual builders in lib/mechanicalVisuals.mjs.
  // Each returns the question AND a structured `visual` spec the UI renders as a
  // precise SVG diagram (predict → reveal). inclinedPlane and forceMotion below
  // remain text-only for now. All physics is unit-tested in tests/mechanical.test.mjs.
  levers: (rng, difficulty) => VISUAL_BUILDERS.levers(rng, difficulty),
  pulleys: (rng, difficulty) => VISUAL_BUILDERS.pulleys(rng, difficulty),
  gears: (rng, difficulty) => VISUAL_BUILDERS.gears(rng, difficulty),
  belts: (rng, difficulty) => VISUAL_BUILDERS.belts(rng, difficulty),
  hydraulics: (rng, difficulty) => VISUAL_BUILDERS.hydraulics(rng, difficulty),
  balance: (rng, difficulty) => VISUAL_BUILDERS.balance(rng, difficulty),
...
// ============================================================================
const math = {
  // Fractions, ratios, and percentages delegate to the diagnostic builders
  // (lib/mathDiagnosis.mjs): each wrong option is tied to a specific, common
  // misconception so a wrong pick yields a "Likely issue: …" hypothesis.
  fractions: (rng, difficulty) => MATH_DIAGNOSIS_BUILDERS.fractions(rng, difficulty),
  ratios: (rng, difficulty) => MATH_DIAGNOSIS_BUILDERS.ratios(rng, difficulty),
  percentages: (rng, difficulty) => MATH_DIAGNOSIS_BUILDERS.percentages(rng, difficulty),
...
const reading = {
  sop(rng, difficulty) {
    ...
    return withReadingCoaching({ passage, prompt: q.prompt, options, correct, explanation: q.explanation }, readingSkillFor(q.prompt));
  },
...
export const SUBSKILLS = {
  mechanical: {
    ...
    subskills: {
      ...
      belts: { label: 'Belt Systems', gen: mechanical.belts },
      hydraulics: { label: 'Hydraulics', gen: mechanical.hydraulics },
      balance: { label: 'Load Balance', gen: mechanical.balance },
  ...
  reading: {
    label: 'Technical Written Materials',
    subskills: {
  ...
  recall: {
    label: 'Verbal & Visual Information',
    subskills: {
      observation: { label: 'Observation Recall', gen: recall.observation },
      sequence: { label: 'Sequence Recall', gen: recall.sequence },
      personnel: { label: 'Personnel Recall', gen: recall.tracking },
      equipment: { label: 'Equipment Recall', gen: recall.observation },
      directional: { label: 'Directional Recall', gen: recall.directional },
      dispatch: { label: 'Dispatch Recall', gen: recall.observation },
    },
  },
};
  ...
  generate({ domain, subskill, difficulty = 2, count = 1, rng = makeRng() }) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      out.push({
        ...
        visual: q.visual || null,   // structured mechanical-diagram spec (or null)
        // Preserve any generator coaching fields (diagnostics, readingSkill,
        // isRecall, …) and ADD governed provenance so every item is auditable
        // against the official blueprint (domain code, objective, origin, no-calc).
        meta: { ...(q.meta || {}), provenance: provenanceFor(domain, sk) },
      });
    }
    return out;
  }
