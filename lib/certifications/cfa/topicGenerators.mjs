...
const shuffle3 = (rng, a) => { const r = [...a]; for (let i = r.length - 1; i > 0; i--) { const j = Math.floor(rng() * (i + 1)); [r[i], r[j]] = [r[j], r[i]]; } return r; };

// Build 3 shuffled options; distractors = [{ value, why, key? }]. Keeps each option's
// rationale (and optional stable misconception key) aligned to its final index (the
// correct option's rationale/key = null). `misconceptions[i]` feeds the cross-question
// misconception memory when a learner picks that option.
function assemble3(rng, correct, distractors, format) {
  const entries = [{ value: correct, why: null, key: null, correct: true }];
  const seen = new Set([format(correct)]);
  for (const d of distractors) {
    const f = format(d.value);
    if (seen.has(f)) continue;
    seen.add(f);
    entries.push({ value: d.value, why: d.why, key: d.key || null, correct: false });
  ...
  while (entries.length < 3 && g++ < 40) {
    ...
    const f = format(v);
    if (!seen.has(f)) { seen.add(f); entries.push({ value: v, why: 'common miscalculation', key: null, correct: false }); }
  }
  ...
  return {
    ...
    misconceptions: entries.map((e) => (e.correct ? null : e.key)),
  };
}
...
export const TOPIC_GENERATORS = {
  ...
  'corpFinance:concepts': (rng) => {
    const mode = pick(rng, ['npv', 'decision']);
    if (mode === 'npv') {
    ...
    // DECISION family (consequence): reason from IRR vs required return to a decision.
    const irr = pick(rng, [0.06, 0.09, 0.11, 0.14]);
    const req = pick(rng, [0.08, 0.10, 0.12]);
    const accept = irr > req;
    const { options, correct, distractorRationale } = assemble3(rng, accept ? 'Accept — its IRR exceeds the required return' : 'Reject — its IRR is below the required return', [
      { value: accept ? 'Reject — its IRR is below the required return' : 'Accept — its IRR exceeds the required return', why: 'reversed the IRR vs required-return rule' },
      { value: 'It cannot be decided from IRR alone', why: 'for a conventional independent project, IRR vs the hurdle rate is decisive' },
    ], (x) => x);
    return { concept: 'npv-decision', prompt: `An independent project has an IRR of ${pct(irr)} and the firm’s required return (hurdle rate) is ${pct(req)}. Should the firm accept it?`, options, correct, explanation: `Accept an independent project when IRR > required return. Here IRR ${pct(irr)} ${accept ? '>' : '<'} ${pct(req)}, so ${accept ? 'accept' : 'reject'} (this agrees with the NPV rule for conventional cash flows).`, meta: { distractorRationale } };
  },
  ...
  'fixedIncome:concepts': (rng) => {
    ...
    const { options, correct, distractorRationale, misconceptions } = assemble3(rng, 'The bond’s price falls', [{ value: 'The bond’s price rises', why: 'price and yield move inversely', key: 'bond-price-yield-inverse' }, { value: 'The bond’s price is unchanged', why: 'fixed-coupon bond prices are sensitive to yields' }], (x) => x);
    return { concept: 'price-yield-inverse', prompt: 'If market interest rates rise, what happens to the price of an existing fixed-coupon bond?', options, correct, explanation: 'Bond prices and yields move inversely: when rates rise, existing fixed-coupon bond prices fall.', meta: { distractorRationale, misconceptions } };
  },
  ...
  'altInvestments:concepts': (rng) => {
    const mode = pick(rng, ['categories', 'characteristics']);
    if (mode === 'categories') {
      // Vary which alternative is the "traditional" odd-one-out and the distractor pool.
      const alts = [
        { value: 'A private equity buyout fund', why: 'private equity is a core alternative investment' },
        { value: 'A direct real estate holding', why: 'real assets like real estate are alternatives' },
        { value: 'A hedge fund', why: 'hedge funds are a core alternative investment' },
        { value: 'A commodities futures position', why: 'commodities are an alternative asset' },
      ];
      const two = shuffle3(rng, alts).slice(0, 2);
      const traditional = pick(rng, ['A publicly traded large-cap stock', 'A U.S. Treasury bond', 'A money-market fund']);
      const { options, correct, distractorRationale } = assemble3(rng, traditional, two, (x) => x);
      return { concept: 'alt-categories', prompt: 'Which of the following is generally NOT classified as an alternative investment?', options, correct, explanation: `${traditional} is a TRADITIONAL investment. Private equity, real assets/real estate, hedge funds, and commodities are alternatives.`, meta: { distractorRationale } };
    }
    // CHARACTERISTICS family (consequence): reason about how alternatives differ from traditional.
    const feats = [
      { value: 'Lower liquidity than public stocks and bonds', why: 'alternatives are typically LESS liquid, not more' },
      { value: 'Higher and often performance-based fees', why: 'alternatives usually carry higher fees' },
      { value: 'Less regulation and transparency', why: 'alternatives are generally less transparent/regulated' },
    ];
    const correctFeat = pick(rng, feats);
    const { options, correct, distractorRationale } = assemble3(rng, correctFeat.value, [
      { value: 'Daily liquidity and continuous public pricing', why: 'that describes traditional public markets, not most alternatives' },
      { value: 'Lower fees than index funds', why: 'alternatives typically charge more, not less' },
    ], (x) => x);
    return { concept: 'alt-characteristics', prompt: 'Relative to traditional stocks and bonds, which is a typical characteristic of alternative investments?', options, correct, explanation: 'Alternatives generally feature lower liquidity, higher (often performance-based) fees, and less transparency/regulation than traditional public investments.', meta: { distractorRationale } };
  },
