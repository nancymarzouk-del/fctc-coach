...
// one CFA profile. `learnerKey` is the opaque handoff id (alphanumeric); direct
// standalone visitors (no key) use the base key. Mirrors FCTC's profile namespace.
export function cfaStorageKey(learnerKey) {
...
export function recordCfaAnswer(state, { domain, subskill, correct, difficulty, misconceptionKey = null, family = null, concept = null }) {
  ...
  // Transfer-mastery evidence: success across DIFFERENT question families per topic.
  if (family) {
    next.transfer = next.transfer || {};
    next.transfer[domain] = next.transfer[domain] || {};
    const fam = next.transfer[domain][family] = next.transfer[domain][family] || { correct: 0, total: 0 };
    fam.total += 1;
...
export function loadCfaState(learnerKey) {
  try {
    if (typeof localStorage === 'undefined') return emptyCfaState();
    const raw = localStorage.getItem(cfaStorageKey(learnerKey));
...
export function saveCfaState(state, learnerKey) {
  try { if (typeof localStorage !== 'undefined') localStorage.setItem(cfaStorageKey(learnerKey), JSON.stringify(state)); } catch { /* ignore */ }
}
