...
export const CFA_GENERATORS = {
  // quant:tvm rotates CALC items AND a CONCEPTUAL (non-calc) item, so quant mastery
  // requires transfer beyond plugging the formula.
  'quant:tvm': (rng) => rotate(rng, [TVM_GENERATORS.singleSumFV, TVM_GENERATORS.singleSumPV, TVM_GENERATORS.annuityPV, TVM_GENERATORS.directionConcept])(rng),
...
export function generateCfaItem({ topic, subskill, rng }) {
  ...
  return {
    ...
    subskill,
    concept: q.concept,
    family: familyOf(q.concept),   // reasoning path — drives anti-repetition + transfer mastery
    prompt: q.prompt,
    options: q.options,
    correct: q.correct,
...
// Generate an item for a cell while AVOIDING recently-seen concepts where the cell
// can produce alternatives — the anti-memorization picker. Falls back to the last
// candidate for single-structure cells. Deterministic given rng.
export function generateVariedItem({ topic, subskill, rng, avoidConcepts = [] }) {
  let last = null;
  for (let i = 0; i < 6; i++) {
    const item = generateCfaItem({ topic, subskill, rng });
    if (!item) return null;
    last = item;
    if (!avoidConcepts.includes(item.concept)) return item;
  }
  return last; // every candidate collided (single-structure cell): accept it
}
...
// `transfer` (from the store) = { topic: { family: { correct, total } } }. Mastery
// requires TRANSFER: a topic is "Strong" only when it is evaluated-high AND the
// learner has answered ≥2 DIFFERENT question families correctly — so grinding one
// recognizable structure can't earn "Strong" (it caps at Developing, needsTransfer).
export function cfaTopicAnalysis(domains, misconceptions, transfer = {}) {
  const registry = metricsRegistryFor(CERT_ID);
  const analysis = analyzeSkills(domains || {}, registry);
  const byTopic = {};
  for (const c of analysis) {
  ...
  const familiesPassed = (t) => {
    const fam = (transfer && transfer[t]) || {};
    return Object.keys(fam).filter((f) => (fam[f] && fam[f].correct) > 0).length;
  };
  // Learner-facing trend for a topic, aggregated ONLY from evaluated subskills that
  // have a real direction (the metrics engine returns 'na' until MIN_EVIDENCE, so
  // sparse topics get no trend claim). Conservative: mixed up+down reads as steady,
  // never a negative claim on ambiguous evidence. Returns 'improving' | 'declining'
  // | 'flat' | null (null → no chip).
  const trendOf = (t) => {
    const dirs = analysis
      .filter((c) => c.domain === t && c.evidenceState === EVIDENCE.EVALUATED && c.trend && c.trend.trend !== 'na')
      .map((c) => c.trend.trend);
  ...
  for (const t of TOPIC_ORDER) {
    ...
    const m = arr.reduce((a, b) => a + b, 0) / arr.length;
    const nFam = familiesPassed(t);
    if (m >= STRONG_THRESHOLD && nFam >= 2) strong.push({ topic: t, mastery: m, families: nFam });
    else if (m >= STRONG_THRESHOLD) developing.push({ topic: t, mastery: m, needsTransfer: true });
    else if (m < WEAK_THRESHOLD) focus.push({ topic: t, mastery: m });
    else developing.push({ topic: t, mastery: m });
  }
  // Attach evidence-gated trend to each evaluated row (untested topics get none).
  for (const row of [...strong, ...focus, ...developing]) row.trend = trendOf(row.topic);
  ...
  if (!hasEnoughEvidence) {
    ...
    const transferNote = top.needsTransfer
      ? ` You're calculating it well — let's confirm you can apply ${TOPICS[top.topic].label} a different way (a concept or scenario question) before calling it mastered.`
      : ` A little more practice should move it into your strengths.`;
    recommendation = { kind: 'practice-topic', topic: top.topic, text: `You're developing in ${TOPICS[top.topic].label}.${transferNote}` };
  } else {
