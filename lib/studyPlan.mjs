...
// `calculator` is optional (e.g., an approved CFA model) and is never chosen for
// the learner — until set, instruction stays calculator-generic.
export function createStudyPlan({ certId, examDate = null, weeklyMinutes = 0, topicPriorities = {}, calculator = null } = {}) {
  return {
    ...
    calculator: calculator || null,      // e.g., "TI BA II Plus" | "HP 12C" | null
  };
}
...
export function updateStudyPlan(plan, patch = {}) {
  ...
  if ('calculator' in patch) next.calculator = patch.calculator || null;
  return next;
}
