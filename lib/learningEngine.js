...
// A full mock-exam plan: `total` questions across the text-MCQ domains
// (mechanical, math, reading), covering every subskill at mixed difficulty and
// deterministically interleaved. Recall is assessed via the timed visual drill,
// not here. Pass a seeded rng for reproducibility.
export function buildMockPlan(rng, total = 100) {
  const domainSubskills = {};
  for (const [dKey, dDef] of Object.entries(SUBSKILLS)) {
    if (dKey === 'recall') continue; // recall is trained through the visual drill
    domainSubskills[dKey] = Object.keys(dDef.subskills);
  }
  return mockBlueprint(domainSubskills, { total, rng });
}
