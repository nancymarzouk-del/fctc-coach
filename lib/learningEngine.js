...
import { officialWeights } from './fctcBlueprint.mjs';
import {
  HISTORY_CAP,
  EVIDENCE,
  masteryEstimate,
...
export function blankState(userId) {
  ...
  return {
    ...
    domains,
    domainStats,
    recallMisses: {},       // { detailType: count } — steers future recall drills
    lastActivity: null,     // { kind, label, domain? } — powers "continue where you left off"
  };
}
...
// A full mock-exam plan across ALL FOUR official domains, at the official FCTC
// section weighting (A Verbal/Visual 20 · B Mechanical 25 · C Math 20 · D
// Technical Written 35), covering every subskill at mixed difficulty and
// deterministically interleaved. Recall (Domain A) is INCLUDED — excluding it was
// the biggest driver of the mock feeling unrepresentative. Weighting comes from
// the governed blueprint (lib/fctcBlueprint.mjs). Pass a seeded rng to reproduce.
export function buildMockPlan(rng, total = 100) {
  const domainSubskills = {};
  for (const [dKey, dDef] of Object.entries(SUBSKILLS)) {
    domainSubskills[dKey] = Object.keys(dDef.subskills);
  }
  return mockBlueprint(domainSubskills, { total, rng, weights: officialWeights() });
}
