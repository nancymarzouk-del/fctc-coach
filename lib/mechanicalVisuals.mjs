...
// Compare TWO pulley systems side by side. Same load, different mechanical
// advantage, so the learner reasons about which needs less effort / has greater
// MA / only changes direction / multiplies force. Deterministic and reuses the
// same strand-count = MA rule as the single-system diagram.
export function makePulleyCompareItem(rng, difficulty) {
  const load = randint(rng, 12, 36) * 10;
  const scenario = pick(rng, ['fixedVsCompound', 'twoCompound']);
  let a, b;
  if (scenario === 'fixedVsCompound') {
    a = { system: 'fixed', strands: 1, load };
    b = { system: 'compound', strands: randint(rng, 2, 4), load };
  } else {
    const s1 = randint(rng, 2, 3);
    const s2 = randint(rng, s1 + 1, 5);
    a = { system: 'compound', strands: s1, load };
    b = { system: 'compound', strands: s2, load };
  }
  const swap = rng() < 0.5;
  const left = swap ? b : a;
  const right = swap ? a : b;
  const maL = left.strands, maR = right.strands;
  const effortL = Math.round(load / maL), effortR = Math.round(load / maR);
  const higher = maL > maR ? 'System A' : 'System B';   // more strands = higher MA
  const lower = maL > maR ? 'System B' : 'System A';
  const fixedSide = left.system === 'fixed' ? 'System A' : (right.system === 'fixed' ? 'System B' : null);

  const variants = scenario === 'fixedVsCompound'
    ? ['lessEffort', 'greaterMA', 'directionOnly', 'forceMult']
    : ['lessEffort', 'greaterMA'];
  const variant = pick(rng, variants);

  let prompt, correct;
  if (variant === 'lessEffort') { prompt = 'Both systems lift the same load. Which requires LESS effort?'; correct = higher; }
  else if (variant === 'greaterMA') { prompt = 'Which system has the greater mechanical advantage?'; correct = higher; }
  else if (variant === 'directionOnly') { prompt = 'Which system only changes the direction of the force (no force multiplication)?'; correct = fixedSide; }
  else { prompt = 'Which system multiplies your force (gives a real lifting advantage)?'; correct = higher; }

  const { options, correct: correctIdx } = assemble(rng, correct, ['System A', 'System B', 'Both are equal', 'Neither'].filter((o) => o !== correct), (x) => x);

  const config = { left, right };
  const answerKey = {
    left: { ...left, ma: maL, effort: effortL }, right: { ...right, ma: maR, effort: effortR },
    winner: correct,
  };
  return {
    prompt, options, correct: correctIdx,
    explanation: `System A has ${maL} supporting strand${maL === 1 ? '' : 's'} (MA ${maL}, effort ${effortL} lbs); System B has ${maR} (MA ${maR}, effort ${effortR} lbs). More supporting strands means greater mechanical advantage and less effort. A single fixed pulley (MA 1) only changes direction.`,
    visual: {
      type: 'pulleyCompare', config, answerKey,
      reveal: { highlight: ['supportingStrands'], caption: `Count the supporting strands: A = ${maL} (effort ${effortL} lbs), B = ${maR} (effort ${effortR} lbs). More strands → higher MA → less effort.` },
    },
  };
}

export function makePulleyItem(rng, difficulty) {
  // At higher difficulty, sometimes pose a side-by-side comparison of two systems.
  if (difficulty >= 3 && rng() < 0.4) return makePulleyCompareItem(rng, difficulty);
...
export function makeLeverItem(rng, difficulty) {
  ...
  if (variant === 'class') {
    ...
    // Exactly the three real lever classes (no padded 4th "class") — shuffled.
    const classes = shuffle(rng, [1, 2, 3]);
    const options = classes.map((k) => `${['', 'First', 'Second', 'Third'][k]}-class lever`);
    const correct = classes.indexOf(klass);
...
export function makeBalanceItem(rng, difficulty) {
  ...
  // Exactly the three natural outcomes (no padded filler) — a beam either balances
  // or tips one way. Shuffled deterministically; correct index reported.
  const options = shuffle(rng, ['It stays balanced', 'It tips left', 'It tips right']);
  const correct = options.indexOf(label);
  const item = {
